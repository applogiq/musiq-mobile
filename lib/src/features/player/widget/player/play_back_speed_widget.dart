...
class PlaybackSheet extends StatefulWidget {
  const PlaybackSheet({super.key, required this.player});
...
class _PlaybackSheetState extends State<PlaybackSheet> {
  ...
  final List<double> _speedList = [0.5, 0.7, 1.0, 1.3, 1.5];
  var _selectedSpeedIndex = 2;
  @override
  void initState() {
    super.initState();
    initialMethod();
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      // log(snapshot.data!.toStringAsFixed(1).toString());

                      return SpeedList(
                        speedText: '${_speedList[index]}x',
                        speedIndex: index,
                        onTap: () async {
                          var playbackSpeed = widget.player.speed;
                          log("123$playbackSpeed");

                          setState(() {
                            _selectedSpeedIndex = index;
                            widget.player.setSpeed(_speedList[index]);
                          });
                          await securetorage.write(
                              key: "speed", value: index.toString());
