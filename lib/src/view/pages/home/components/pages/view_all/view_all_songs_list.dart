...
class _ViewAllScreenSongListState extends State<ViewAllScreenSongList> {
  ...
  loadData() {
    ...
    print(widget.view_all_song_list_model[0].songId);
    for (int i = 0; i < widget.view_all_song_list_model.length; i++) {
      playScreenModel.add(PlayScreenModel(
          id: int.parse(widget.view_all_song_list_model[i].songId),
          songName: widget.view_all_song_list_model[i].songName,
          musicDirectorName:
              widget.view_all_song_list_model[i].songMusicDirector,
          albumId: widget.view_all_song_list_model[i].album_id,
          albumName: widget.view_all_song_list_model[i].albumName));
    }
    print(playScreenModel.length);
    Navigator.of(context).push(MaterialPageRoute(
        builder: (context) =>
            MainPlayScreen(playScreenModel: playScreenModel, index: 0)));
  }
            ...
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  ...
                  for (int i = 0;
                      i < widget.view_all_song_list_model.length;
                      i++) {
                    playScreenModel.add(PlayScreenModel(
                        id: int.parse(
                            widget.view_all_song_list_model[i].songId),
                        songName: widget.view_all_song_list_model[i].songName,
                        musicDirectorName: widget
                            .view_all_song_list_model[i].songMusicDirector,
                        albumId: widget.view_all_song_list_model[i].album_id,
                        albumName:
                            widget.view_all_song_list_model[i].albumName));
                  }
                  print(playScreenModel.length);
                  Navigator.of(context).push(MaterialPageRoute(
