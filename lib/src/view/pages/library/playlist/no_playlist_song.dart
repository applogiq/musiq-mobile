...
class NoPlaylistSong extends StatelessWidget {
   NoPlaylistSong({Key? key, required this.appBarTitle, required this.playListId}) : super(key: key);
  final String appBarTitle;
  final String playListId;
    LibraryController libraryController = Get.put(LibraryController());
  
PopupMenuItem _buildPopupMenuItem(String title, value,context,  {int position = 0,int id=0,}) {
    return PopupMenuItem(
      onTap: ()async{
       ...
       Future.delayed(
                    ...
                    () => showDialog(
                          context: context,
                          builder: (context) => CustomDialogBox(initialText: appBarTitle, isRename: true,title: ConstantText.renamePlaylist, fieldName: "Name", buttonText: "Rename"),
                          ),
                        );
        }
      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomInset: false,
            appBar: PreferredSize(preferredSize: Size(MediaQuery.of(context).size.width, 80),child:AppBar( leading: InkWell(
          onTap: () {
 ...
 body: Column(
  ...
  children: [
    Spacer(),
      NoSongScreen(isFav: true,mainTitle: ConstantText.noSongHere,subTitle: ConstantText.noSongInPlayList,),
      InkWell(onTap: (){
        Navigator.of(context).push(MaterialPageRoute(builder: (context)=>SearchScreen()));
      },child: CustomButton(label: ConstantText.browseSong,horizontalMargin: 105,))
     , Spacer()
     ,Spacer()
