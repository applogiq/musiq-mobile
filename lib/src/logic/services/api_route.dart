...
class APIRoute {
  ...
  Future<RecentlyPlayed> getRecentlyPlayed({int limit = 100}) async {
    ...
    if (res.statusCode == 404) {
      RecentlyPlayed recentlyPlayed = RecentlyPlayed(
          success: false, message: "NO", records: [], totalrecords: 0);
  ...
  Future<Album> getAlbum({int limit = 100}) async {
    ...
    var res =
        await apiCall.getRequestWithAuth(APIConstants.BASE_URL + albumsUrl);
    var data = jsonDecode(res.body);
    print(data.toString());
  ...
  getSpecificPlaylist(int? id) async {
    var res = await apiCall.getRequestWithAuth(
        APIConstants.BASE_URL + APIConstants.SPECIFIC_PLAYLIST + id.toString());
    print(res.statusCode);
    if (res.statusCode == 200) {
      print(jsonDecode(res.body));
      PlayListSongModel playListModel =
          PlayListSongModel.fromMap(jsonDecode(res.body));
      return playListModel;
    } else {
      PlayListSongModel playListModel = PlayListSongModel(
  ...
  getSpecificArtistSong(
      {required int index, int skip = 0, int limit = 100}) async {
    ...
    if (res.statusCode == 200) {
      ...
      SongList songList = SongList(
          success: false, message: "No Data", records: [], totalrecords: 0);
      return songList;
    }
  }
  ...
  getSpecificAuraSongs({required int id, int limit = 100}) async {
    ...
    var res = await apiCall
        .getRequestWithAuth(APIConstants.BASE_URL + auroSpecificUrl);
    var data = jsonDecode(res.body);
    print(data.toString());
  ...
  getSpecificAlbumSong({required int id, int limit = 100}) async {
    ...
    var res =
        await apiCall.getRequestWithAuth(APIConstants.BASE_URL + albumSpecific);
    var data = jsonDecode(res.body);
    print(data.toString());
...
// http://192.168.29.185:3000/api/v1/aura/?limit=100
  ...
  getTrendingHits({int limit = 100}) async {
    ...
    var res = await apiCall
        .getRequestWithAuth(APIConstants.BASE_URL + trendingHitsUrl);
    if (res.statusCode == 200) {
      var data = jsonDecode(res.body);
      print(data.toString());
  ...
  getNewRelease({required int limit}) async {
    ...
    var res =
        await apiCall.getRequestWithAuth(APIConstants.BASE_URL + newReleaseUrl);
    if (res.statusCode == 200) {
      var data = jsonDecode(res.body);
      print(data.toString());
      ...
      TrendingHitsModel new_release = TrendingHitsModel(
          success: false, message: "no data", records: [], totalrecords: 0);
      return new_release;
    }
  }
  ...
  profileUpdate(Map params) async {
    var id = await storage.read(key: "id");
    print(id);
    String url = APIConstants.USER + id.toString();
    print(url);
